...
ResultSet rt = this.getStatement().executeQuery("select * from login where UN='"+ValidateSQLi(U)+"' and PD='"+this.ValidateSQLi(P)+"'");
return rt.next();
}
...
private String ValidateSQLi(String test)
{
    test=test.replace("'", "");
    return test;
}
